...
##################################################
# function to read in data from the .arff files
##################################################
def read_arffdata(dataset, datafile):

    data = arff.load(open(datafile, 'rb'))

    dataset.examples = np.array(data['data']).tolist()

    dataset.attributes = [i[0] for i in data['attributes']]

    dataset.attr_types = ['true' for i in data['attributes']]

    dataset.attr_types[-1] = 'false'
...
##################################################
# Computing tree model
##################################################
def train_model(datafile, classifiername=None):

    filename, file_extension = os.path.splitext(datafile)
    if (file_extension != ".arff"):
        print "Your training file (second argument) must be a .arff!"
    else:
        dataset = data("")
        read_arffdata(dataset, datafile)
        if (classifiername in dataset.attributes):
        ...
        return root, dataset, classifier
...
def main():
    ...
    filename, file_extension = os.path.splitext(args[1])
    if (len(args) < 2):
        ...
        root, dataset, classifier = train_model(args[1], args[2])
        ...
        if ("-v" in args):
            ...
            read_arffdata(validateset, datavalidate)
        ...
        if ("-t" in args):
            ...
            read_arffdata(testset, datatest)
            for a in range(len(dataset.attributes)):
            ...
            for example in testset.examples:
                print test_example(example, root, testset.class_index)
